...
double* LinerRegression::getCoef()
{
    coef = new double[result->_solution_colunm_size * result->_solution_row_size];
    transpose(result->_lstsq_solution,result->_solution_colunm_size,result->_solution_row_size,coef);
    return coef;
}

void OLS::fit(double* Matrix_A, int Matrix_A_colunm, int Matrix_A_row,double* Matrix_B, int Matrix_B_colunm, int Matrix_B_row)
{
    result = new lstsq_result(Matrix_B_colunm,Matrix_B_row);
